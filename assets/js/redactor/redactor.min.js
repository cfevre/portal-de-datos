...
jQuery.fn.redactor = function(a) {
    return this.each(function() {
        var c = $(this);
        var b = c.data("redactor");
        if (!b) {
            c.data("redactor", (b = new Redactor(this, a)))
        }
    })
};
var Redactor = function(b, a) {
    this.$el = $(b);
    if (typeof a !== "undefined" && typeof a.lang !== "undefined" && a.lang !== "en" && typeof RELANG[a.lang] !== "undefined") {
        RLANG = RELANG[a.lang]
    }
    this.opts = $.extend({
        lang: "en",
        ...
        observeImages: true,
        overlay: true,
        allowedTags: ["code", "span", "div", "label", "a", "br", "p", "b", "i", "del", "strike", "img", "video", "audio", "iframe", "object", "embed", "param", "blockquote", "mark", "cite", "small", "ul", "ol", "li", "hr", "dl", "dt", "dd", "sup", "sub", "big", "pre", "code", "figure", "figcaption", "strong", "em", "table", "tr", "td", "th", "tbody", "thead", "tfoot", "h1", "h2", "h3", "h4", "h5", "h6"],
        buttonsCustom: {},
        buttonsAdd: [],
        ...
        buffer: false,
        visual: true,
        modal_file: String() + '<form id="redactorUploadFileForm" method="post" action="" enctype="multipart/form-data"><label>Name (optional)</label><input type="text" id="redactor_filename" class="redactor_input" /><div style="margin-top: 7px;"><input type="file" id="redactor_file" name="file" /></div></form>',
...
Redactor.prototype = {
    init: function() {
        ...
        if (this.opts.air) {
            ...
            if (this.opts.toolbar !== false) {
                ...
                $.each(this.opts.buttonsAdd, $.proxy(function(a, b) {
        ...
        if (this.isMobile(true) === false) {
            this.$editor.bind("paste", $.proxy(function(a) {
                this.setBuffer();
                var b = this.extractContent();
                setTimeout($.proxy(function() {
                    var c = this.extractContent();
                    this.$editor.append(b);
                    this.restoreSelection();
                    var d = this.getFragmentHtml(c);
                    this.pasteCleanUp(d)
                }, this), 1)
            }, this))
        }
    ...
    shortcuts: function(b, a) {
        b.preventDefault();
        this.execCommand(a, false)
    },
    keyup: function() {
        this.$editor.keyup($.proxy(function(b) {
            var a = b.keyCode || b.which;
            if (typeof this.opts.keyupCallback === "function") {
                this.opts.keyupCallback(this, b)
            }
            if (a === 8 || a === 46) {
                this.observeImages();
                return this.formatEmpty(b)
            }
            if (a === 13 && !b.shiftKey && !b.ctrlKey && !b.metaKey) {
                return this.formatNewLine(b)
            }
            this.syncCode()
        }, this))
    },
    keydown: function() {
        this.$editor.keydown($.proxy(function(f) {
            var a = f.keyCode || f.which;
            var b = this.getParentNode();
            var d = false;
            var c = f.ctrlKey || f.metaKey;
            if (b && $(b).get(0).tagName === "PRE") {
                d = true
            }
            if (typeof this.opts.keydownCallback === "function") {
                this.opts.keydownCallback(this, f)
            }
            if (c) {
                if (a === 90) {
                    if (this.opts.buffer !== false) {
                        ...
                        this.shortcuts(f, "undo")
                    }
                } else {
                    if (a === 90 && f.shiftKey) {
                        this.shortcuts(f, "redo")
                    } else {
                        if (a === 77) {
                            this.shortcuts(f, "removeFormat")
                        } else {
                            if (a === 66) {
                                this.shortcuts(f, "bold")
                            } else {
                                if (a === 73) {
                                    this.shortcuts(f, "italic")
                                } else {
                                    if (a === 74) {
                                        this.shortcuts(f, "insertunorderedlist")
                                    } else {
                                        if (a === 75) {
                                            this.shortcuts(f, "insertorderedlist")
                                        } else {
                                            if (a === 76) {
                                                this.shortcuts(f, "superscript")
                                            } else {
                                                if (a === 72) {
                                                    this.shortcuts(f, "subscript")
                                                }
                                            }
                                        }
                                    }
            ...
            if (!f.shiftKey && a === 9) {
                ...
                if (f.shiftKey && a === 9) {
                    this.shortcuts(f, "outdent")
                }
            }
            if ($.browser.webkit && navigator.userAgent.indexOf("Chrome") === -1) {
                return this.safariShiftKeyEnter(f, a)
            }
        }, this))
    },
    build: function(b) {
        if (b !== true) {
            ...
            if (this.$el.get(0).tagName !== "TEXTAREA") {
                var c = this.$el.val();
                var a = $('<textarea name="' + this.$editor.attr("id") + '"></textarea>').css("height", this.height).val(c);
                this.$el.hide();
                this.$el.after(a)
            }
        }
    ...
    enableAir: function() {
        ...
        this.air.hide();
        this.$editor.bind("textselect", $.proxy(function(a) {
            this.showAir(a)
        }, this));
    ...
    showAir: function(c) {
        ...
        var b = c.clientX;
        if ($(document).width() < (b + a)) {
            b = b - a
        }
        this.air.css({
            left: b + "px",
            top: (c.clientY + $(document).scrollTop() + 14) + "px"
        }).show()
    },
    syncCode: function() {
    ...
    setCode: function(a) {
        this.$editor.html(a).focus();
        this.syncCode()
    },
    getCode: function() {
        var a = this.$editor ? this.$editor.html() : this.$el.val();
        return a
    },
    ...
    destroy: function() {
        ...
        if (this.textareamode) {
            ...
            this.$box.remove();
            this.$editor.removeClass("redactor_editor").removeClass("redactor_editor_wym").attr("contenteditable", false).html(b).show()
        }
        for (var a = 0; a < this.dropdowns.length; a++) {
            this.dropdowns[a].remove();
            delete(this.dropdowns[a])
        }
    },
    observeImages: function() {
        ...
        this.$editor.find("img").each($.proxy(function(a, b) {
            if ($.browser.msie) {
    ...
    execCommand: function(b, d) {
        try {
            var a;
            if (b === "inserthtml" && $.browser.msie) {
                document.selection.createRange().pasteHTML(d)
            } else {
                if (b === "formatblock" && $.browser.msie) {
                    ...
                    if (b === "unlink") {
                        ...
                        if (b === "formatblock" && d === "blockquote") {
                            a = this.getParentNode();
                            if ($(a).get(0).tagName === "BLOCKQUOTE") {
                                document.execCommand(b, false, "p")
                            } else {
                                document.execCommand(b, false, d)
                            }
                        } else {
                            if (b === "formatblock" && d === "pre") {
                                a = this.getParentNode();
                                if ($(a).get(0).tagName === "PRE") {
                                    $(a).replaceWith("<p>" + $(a).text() + "</p>")
                                } else {
                                    document.execCommand(b, false, d)
                                }
                            } else {
            ...
            if (typeof this.opts.execCommandCallback === "function") {
                this.opts.execCommandCallback(this, b)
            }
            if (this.opts.air) {
                this.air.hide()
            }
    ...
    formatNewLine: function(c) {
        if ($.browser.webkit) {
            var b = this.getParentNode();
            if (b.nodeName === "DIV" && b.className === "redactor_editor") {
                c.preventDefault();
                var a = $(this.getCurrentNode());
                if (a.get(0).tagName === "DIV" && (a.html() === "" || a.html() === "<br>")) {
                    var d = $("<p>").append(a.clone().get(0).childNodes);
                    a.replaceWith(d);
                    d.html("<br />");
    ...
    safariShiftKeyEnter: function(c, b) {
        if (c.shiftKey && b === 13) {
            c.preventDefault();
            var a = $("<span><br /></span>");
            this.insertNodeAtCaret(a.get(0));
            this.syncCode();
            return false
        } else {
            return true
    ...
    paragraphy: function(c) {
        ...
        var d = function(f, g, e) {
            return f.replace(new RegExp(g, "g"), e)
        };
        var a = function(f, e) {
            return d(c, f, e)
        };
        var b = "(table|thead|tfoot|caption|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|select|form|blockquote|address|math|style|script|object|input|param|p|h[1-6])";
        c += "\n";
        a("<br />\\s*<br />", "\n\n");
        a("(<" + b + "[^>]*>)", "\n$1");
        a("(</" + b + ">)", "$1\n\n");
        ...
        a("<p>\\s*?</p>", "");
        a("<p>(<div[^>]*>\\s*)", "$1<p>");
        a("<p>([^<]+)\\s*?(</(div|address|form)[^>]*>)", "<p>$1</p>$2");
        a("<p>\\s*(</?" + b + "[^>]*>)\\s*</p>", "$1");
        a("<p>(<li.+?)</p>", "$1");
        a("<p>\\s*(</?" + b + "[^>]*>)", "$1");
        a("(</?" + b + "[^>]*>)\\s*</p>", "$1");
        a("(</?" + b + "[^>]*>)\\s*<br />", "$1");
        a("<br />(\\s*</?(p|li|div|dl|dd|dt|th|pre|td|ul|ol)[^>]*>)", "$1");
        if (c.indexOf("<pre") != -1) {
            a("(<pre(.|\n)*?>)((.|\n)*?)</pre>", function(h, g, f, e) {
                return d(g, "\\\\(['\"\\\\])", "$1") + d(d(d(e, "<p>", "\n"), "</p>|<br />", ""), "\\\\(['\"\\\\])", "$1") + "</pre>"
            })
        }
        return a("\n</p>$", "</p>")
    },
    stripTags: function(b) {
        ...
        return b.replace(a, function(e, d) {
            return $.inArray(d.toLowerCase(), c) > "-1" ? e : ""
        })
    },
    pasteCleanUp: function(a) {
        a = a.replace(/<!--[\s\S]*?-->|<\?(?:php)?[\s\S]*?\?>/gi, "");
        a = a.replace(/(&nbsp;){1,}/gi, "&nbsp;");
        a = a.replace(/<b\sid="internal-source-marker(.*?)">([\w\W]*?)<\/b>/gi, "$2");
        a = this.stripTags(a);
        a = a.replace(/<td><br><\/td>/gi, "[td]");
        a = a.replace(/<a(.*?)>([\w\W]*?)<\/a>/gi, "[a$1]$2[/a]");
        a = a.replace(/<iframe(.*?)>([\w\W]*?)<\/iframe>/gi, "[iframe$1]$2[/iframe]");
        a = a.replace(/<video(.*?)>([\w\W]*?)<\/video>/gi, "[video$1]$2[/video]");
        a = a.replace(/<audio(.*?)>([\w\W]*?)<\/audio>/gi, "[audio$1]$2[/audio]");
        a = a.replace(/<object(.*?)>([\w\W]*?)<\/object>/gi, "[object$1]$2[/object]");
        a = a.replace(/<img(.*?)>/gi, "[img$1]");
        a = a.replace(/<(\w+)([\w\W]*?)>/gi, "<$1>");
        a = a.replace(/<[^\/>][^>]*>(\s*|\t*|\n*|&nbsp;|<br>)<\/[^>]+>/gi, "");
        a = a.replace(/<[^\/>][^>]*>(\s*|\t*|\n*|&nbsp;|<br>)<\/[^>]+>/gi, "");
        a = a.replace(/\[td\]/gi, "<td><br></td>");
        a = a.replace(/\[a(.*?)\]([\w\W]*?)\[\/a\]/gi, "<a$1>$2</a>");
        a = a.replace(/\[iframe(.*?)\]([\w\W]*?)\[\/iframe\]/gi, "<iframe$1>$2</iframe>");
        a = a.replace(/\[video(.*?)\]([\w\W]*?)\[\/video\]/gi, "<video$1>$2[/video>");
        a = a.replace(/\[audio(.*?)\]([\w\W]*?)\[\/audio\]/gi, "<audio$1>$2[/audio>");
        a = a.replace(/\[object(.*?)\]([\w\W]*?)\[\/object\]/gi, "<object$1>$2</object>");
        a = a.replace(/\[img(.*?)\]/gi, "<img$1>");
        a = a.replace(/<div(.*?)>([\w\W]*?)<\/div>/gi, "<p>$2</p>");
    ...
    formattingEmptyTags: function(d) {
        var c = ["<pre></pre>", "<blockquote>\\s*</blockquote>", "<em>\\s*</em>", "<ul></ul>", "<ol></ol>", "<li></li>", "<table></table>", "<tr></tr>", "<span>\\s*<span>", "<span>&nbsp;<span>", "<b>\\s*</b>", "<b>&nbsp;</b>", "<p>\\s*</p>", "<p>&nbsp;</p>", "<p>\\s*<br>\\s*</p>", "<div>\\s*</div>", "<div>\\s*<br>\\s*</div>"];
    ...
    formattingAddBefore: function(c) {
        var d = "\r\n";
        var e = ["<p", "<form", "</ul>", "</ol>", "<fieldset", "<legend", "<object", "<embed", "<select", "<option", "<input", "<textarea", "<pre", "<blockquote", "<ul", "<ol", "<li", "<dl", "<dt", "<dd", "<table", "<thead", "<tbody", "<caption", "</caption>", "<th", "<tr", "<td", "<figure"];
        for (var b = 0; b < e.length; ++b) {
            var a = e[b];
            c = c.replace(new RegExp(a, "gi"), d + a)
        }
        return c
    },
    formattingAddAfter: function(c) {
        var e = "\r\n";
        var a = ["</p>", "</div>", "</h1>", "</h2>", "</h3>", "</h4>", "</h5>", "</h6>", "<br>", "<br />", "</dl>", "</dt>", "</dd>", "</form>", "</blockquote>", "</pre>", "</legend>", "</fieldset>", "</object>", "</embed>", "</textarea>", "</select>", "</option>", "</table>", "</thead>", "</tbody>", "</tr>", "</td>", "</th>", "</figure>"];
        for (var b = 0; b < a.length; ++b) {
            var d = a[b];
            c = c.replace(new RegExp(d, "gi"), d + e)
        }
        return c
    },
    formatting: function(a) {
        a = this.formattingRemove(a);
        a = this.formattingEmptyTags(a);
        a = this.formattingAddBefore(a);
        a = this.formattingAddAfter(a);
        a = this.formattingIndenting(a);
        return a
    },
    toggle: function() {
        var a;
        if (this.opts.visual) {
            this.$editor.hide();
            a = this.$editor.html();
            a = $.trim(this.formatting(a));
            this.$el.val(a).show().focus();
    ...
    buildToolbar: function() {
        ...
        $.each(this.opts.buttons, $.proxy(function(e, d) {
            if (d !== "|" && typeof this.opts.toolbar[d] !== "undefined") {
                var f = this.opts.toolbar[d];
                if (this.opts.fileUpload === false && d === "file") {
                    return true
                }
                var b = $("<li>");
                var c = this.buildButton(d, f);
                if (d === "backcolor" || d === "fontcolor" || typeof(f.dropdown) !== "undefined") {
                    var g = $('<div class="redactor_dropdown" style="display: none;">');
                    if (d === "backcolor" || d === "fontcolor") {
                        g = this.buildColorPicker(g, d)
                    } else {
                        g = this.buildDropdown(g, f.dropdown)
                    }
                    this.dropdowns.push(g.appendTo($(document.body)));
                    this.hdlHideDropDown = $.proxy(function(a) {
                        this.hideDropDown(a, g, d)
                    }, this);
                    this.hdlShowDropDown = $.proxy(function(a) {
                        this.showDropDown(a, g, d)
                    }, this);
                    c.click(this.hdlShowDropDown)
                }
                this.$toolbar.append($(b).append(c))
            }
            if (d === "|") {
    ...
    buildButton: function(b, c) {
        var a = $('<a href="javascript:void(null);" title="' + c.title + '" class="redactor_btn_' + b + '"></a>');
        if (typeof c.func === "undefined") {
            a.click($.proxy(function() {
                this.execCommand(c.exec, b)
            }, this))
        } else {
            if (c.func !== "show") {
                a.click($.proxy(function(d) {
                    if (this.oldIE()) {
                        this.$editor.focus();
                        this.saveSelection()
                    }
    ...
    buildDropdown: function(b, a) {
        $.each(a, $.proxy(function(c, f) {
            if (typeof(f.className) === "undefined") {
                f.className = ""
            }
            var e;
            if (typeof f.name !== "undefined" && f.name === "separator") {
                e = $('<a class="redactor_separator_drop">')
            } else {
                e = $('<a href="javascript:void(null);" class="' + f.className + '">' + f.title + "</a>");
                if (typeof(f.func) === "undefined") {
                    $(e).click($.proxy(function() {
                        this.execCommand(f.exec, c)
                    }, this))
                } else {
                    $(e).click($.proxy(function(d) {
                        this[f.func](d)
                    }, this))
                }
            }
    ...
    buildColorPicker: function(j, h) {
        ...
        $(j).width(210);
        var e = this.opts.colors.length;
        for (var c = 0; c < e; ++c) {
            var a = this.opts.colors[c];
            var g = $('<a rel="' + a + '" href="javascript:void(null);" class="redactor_color_link"></a>').css({
                backgroundColor: a
            });
            $(j).append(g);
            var f = this;
            $(g).click(function() {
                f.execCommand(d, $(this).attr("rel"));
                if (d === "forecolor") {
                    f.$editor.find("font").replaceWith(function() {
                        return $('<span style="color: ' + $(this).attr("color") + ';">' + $(this).html() + "</span>")
                    })
                }
                if ($.browser.msie && d === "BackColor") {
                    f.$editor.find("font").replaceWith(function() {
        ...
        var b = $('<a href="javascript:void(null);" class="redactor_color_none"></a>').html(RLANG.none);
        if (h === "backcolor") {
            b.click($.proxy(this.setBackgroundNone, this))
        } else {
            b.click($.proxy(this.setColorNone, this))
        }
        $(j).append(b);
        return j
    },
    ...
    showDropDown: function(f, g, a) {
        if (this.getBtn(a).hasClass("dropact")) {
            this.hideAllDropDown()
        } else {
            this.hideAllDropDown();
            this.setBtnActive(a);
            this.getBtn(a).addClass("dropact");
            var d = this.getBtn(a).offset().left;
            if (this.opts.air) {
                var b = this.air.offset().top;
                $(g).css({
                    position: "absolute",
                    left: d + "px",
                    top: b + 30 + "px"
                ...
                if (this.opts.fixed && this.fixed) {
                    ...
                    var c = this.$toolbar.offset().top + 30;
                    $(g).css({
                        position: "absolute",
                        left: d + "px",
                        top: c + "px"
    ...
    hideDropDown: function(b, c, a) {
        if (!$(b.target).hasClass("dropact")) {
            $(c).removeClass("act");
    ...
    saveSelection: function() {
        ...
        if ($.browser.msie && parseInt($.browser.version, 10) < 9) {
            var a = this.$editor.get(0);
            this.savedSel = window.Selection.getOrigin(a);
            this.savedSelObj = window.Selection.getFocus(a)
        } else {
    ...
    insertNodeAtCaret: function(e) {
        if (window.getSelection) {
            ...
            if (document.selection) {
                var c = (e.nodeType === 1) ? e.outerHTML : e.data;
                var g = "marker_" + ("" + Math.random()).slice(2);
                c += '<span id="' + g + '"></span>';
        ...
        if (window.getSelection) {
            ...
            if (document.selection) {
                if (document.selection.type === "Text") {
                    d = document.selection.createRange().htmlText
                }
            }
        }
        return d
    ...
    getBtn: function(a) {
        return $(this.$toolbar.find("a.redactor_btn_" + a))
    },
    setBtnActive: function(a) {
        this.getBtn(a).addClass("act")
    },
    setBtnInactive: function(a) {
        this.getBtn(a).removeClass("act")
    },
    resizeImage: function(e) {
        ...
        $(e).mousedown(function(i) {
            ...
            f = true;
            b = true;
            d = Math.round(i.pageX - $(e).eq(0).offset().left);
            a = Math.round(i.pageY - $(e).eq(0).offset().top)
        });
        $(e).mouseup($.proxy(function(i) {
            f = false;
            this.syncCode()
        }, this));
        $(e).click($.proxy(function(i) {
            if (b) {
                this.imageEdit(i)
            }
        }, this));
        $(e).mousemove(function(m) {
            if (f) {
                b = false;
                var j = Math.round(m.pageX - $(this).eq(0).offset().left) - d;
                var i = Math.round(m.pageY - $(this).eq(0).offset().top) - a;
                var l = $(e).height();
                var n = parseInt(l, 10) + i;
                var k = n * g;
                if (k > c) {
                    $(e).width(k)
                }
                ...
                d = Math.round(m.pageX - $(this).eq(0).offset().left);
                a = Math.round(m.pageY - $(this).eq(0).offset().top)
            }
        })
    },
    showTable: function() {
    ...
    insertTable: function() {
        var k = $("#redactor_table_rows").val();
        var d = $("#redactor_table_columns").val();
        var a = $("<div></div>");
        var b = Math.floor(Math.random() * 99999);
        var h = $('<table id="table' + b + '"><tbody></tbody></table>');
        for (var e = 0; e < k; e++) {
            var j = $("<tr></tr>");
            for (var g = 0; g < d; g++) {
                var c = $("<td><br></td>");
    ...
    tableObserver: function(a) {
        this.$table = $(a.target).parents("table");
        this.$table_tr = this.$table.find("tr");
        this.$table_td = this.$table.find("td");
        this.$table_td.removeClass("current");
        this.$tbody = $(a.target).parents("tbody");
        this.$thead = $(this.$table).find("thead");
        this.$current_td = $(a.target);
        this.$current_td.addClass("current");
        this.$current_tr = $(a.target).parents("tr")
    },
    ...
    deleteColumn: function() {
        var a = $(this.$current_td).get(0).cellIndex;
    ...
    addHead: function() {
        if ($(this.$table).find("thead").size() !== 0) {
            ...
            var a = $(this.$table).find("tr").first().clone();
            a.find("td").html("&nbsp;");
    ...
    insertRow: function(a) {
        var b = $(this.$current_tr).clone();
        b.find("td").html("&nbsp;");
        if (a === "after") {
            $(this.$current_tr).after(b)
        } else {
            $(this.$current_tr).before(b)
        }
        this.syncCode()
    },
    insertColumn: function(b) {
        var a = 0;
        this.$current_td.addClass("current");
        this.$current_tr.find("td").each(function(c, d) {
            if ($(d).hasClass("current")) {
                a = c
            }
        });
        this.$table_tr.each(function(c, d) {
            var e = $(d).find("td").eq(a);
            var f = e.clone();
            f.html("&nbsp;");
            if (b === "after") {
                $(e).after(f)
    ...
    imageEdit: function(d) {
        ...
        var b = $.proxy(function() {
            $("#redactor_file_alt").val(a.attr("alt"));
            $("#redactor_image_edit_src").attr("href", a.attr("src"));
            $("#redactor_form_image_align").val(a.css("float"));
            if ($(c).get(0).tagName === "A") {
                $("#redactor_file_link").val($(c).attr("href"))
            }
        ...
        this.modalInit(RLANG.image, "image_edit", 380, b)
    },
    imageDelete: function(a) {
        $(a).remove();
        this.modalClose();
        this.syncCode()
    },
    imageSave: function(b) {
        ...
        var c = $.trim($("#redactor_file_link").val());
        if (c !== "") {
            if ($(a).get(0).tagName !== "A") {
                $(b).replaceWith('<a href="' + c + '">' + this.outerHTML(b) + "</a>")
            } else {
                $(a).attr("href", c)
            }
        } else {
            if ($(a).get(0).tagName === "A") {
                $(a).replaceWith(this.outerHTML(b))
            }
        }
        this.modalClose();
        this.observeImages();
    ...
    showImage: function() {
        ...
        var a = $.proxy(function() {
            if (this.opts.imageGetJson !== false) {
                $.getJSON(this.opts.imageGetJson, $.proxy(function(d) {
                    $.each(d, $.proxy(function(f, g) {
                        var e = $('<img src="' + g.thumb + '" rel="' + g.image + '" />');
                        $("#redactor_image_box").append(e);
                        $(e).click($.proxy(this.imageSetThumb, this))
                    }, this))
                }, this))
            } else {
            ...
            if (this.opts.imageUpload !== false) {
                ...
                if (this.opts.imageGetJson === false) {
                    ...
                    var c = $("#redactor_tabs a");
                    c.eq(0).remove();
                    c.eq(1).addClass("redactor_tabs_act");
                    $("#redactor_tab2").show()
                }
            }
        ...
        this.modalInit(RLANG.image, "image", 570, a, b, true)
    },
    imageSetThumb: function(a) {
        this._imageSet('<img alt="" src="' + $(a.target).attr("rel") + '" />', true)
    },
    imageUploadCallbackLink: function() {
        if ($("#redactor_file_link").val() !== "") {
            var a = '<img src="' + $("#redactor_file_link").val() + '" />';
            this._imageSet(a, true)
        } else {
            this.modalClose()
        }
    },
    ...
    _imageSet: function(b, c) {
        ...
        if (b !== false) {
            ...
            if (c !== true) {
                d = $.parseJSON(b);
                a = '<p><img src="' + d.filelink + '" /></p>'
            } else {
                a = b
            }
            this.execCommand("inserthtml", a);
            if (c !== true && typeof this.opts.imageUploadCallback === "function") {
    ...
    showLink: function() {
        ...
        var a = $.proxy(function() {
            ...
            if ($.browser.msie) {
                ...
                if (e.nodeName === "A") {
                    this.insert_link_node = $(e);
                    h = this.insert_link_node.text();
                    c = this.insert_link_node.attr("href")
                } else {
                    if (this.oldIE()) {
                        h = g.text
                    } else {
                    ...
                    var d = '<span id="span' + this.spanid + '">' + h + "</span>";
                    if (h !== "") {
                        d = '<span id="span' + this.spanid + '">' + h + "</span>"
                    }
                    this.execCommand("inserthtml", d)
                }
            } else {
                if (g && g.anchorNode && g.anchorNode.parentNode.tagName === "A") {
                    c = g.anchorNode.parentNode.href;
                    h = g.anchorNode.parentNode.text;
                    if (g.toString() === "") {
            ...
            $(".redactor_link_text").val(h);
            var f = c.replace(top.location.href, "");
            if (c.search("mailto:") === 0) {
                this.setModalTab(2);
                $("#redactor_tab_selected").val(2);
                $("#redactor_link_mailto").val(c.replace("mailto:", ""))
            } else {
                if (f.search(/^#/gi) === 0) {
                    this.setModalTab(3);
                    $("#redactor_tab_selected").val(3);
                    $("#redactor_link_anchor").val(f.replace(/^#/gi, ""))
                } else {
                    $("#redactor_link_url").val(c)
                }
            }
        ...
        this.modalInit(RLANG.link, "link", 460, a, b)
    },
    insertLink: function() {
        var b = $("#redactor_tab_selected").val();
        var a = "",
            c = "";
        if (b === "1") {
            a = $("#redactor_link_url").val();
            c = $("#redactor_link_url_text").val()
        } else {
            if (b === "2") {
                a = "mailto:" + $("#redactor_link_mailto").val();
                c = $("#redactor_link_mailto_text").val()
            } else {
                if (b === "3") {
                    a = "#" + $("#redactor_link_anchor").val();
                    c = $("#redactor_link_anchor_text").val()
                }
            }
        }
        this._insertLink('<a href="' + a + '">' + c + "</a> ", $.trim(c), a)
    },
    _insertLink: function(b, d, c) {
        this.restoreSelection();
        if (d !== "") {
            if (this.insert_link_node) {
                $(this.insert_link_node).text(d);
                $(this.insert_link_node).attr("href", c);
                this.syncCode()
            } else {
                if ($.browser.msie) {
                    $("#span" + this.spanid).replaceWith(b);
                    this.syncCode()
                } else {
    ...
    showFile: function() {
        ...
        this.modalInit(RLANG.file, "file", 500, a)
    },
    fileUploadCallback: function(a) {
        this.restoreSelection();
        if (a !== false) {
            var c = $.parseJSON(a);
            var d = $("#redactor_filename").val();
            if (d === "") {
                d = c.filename
            }
            var b = '<a href="' + c.filelink + '">' + d + "</a>";
    ...
    modalInit: function(h, c, e, d, g, b) {
        ...
        this.hdlModalClose = $.proxy(function(i) {
            if (i.keyCode === 27) {
                this.modalClose()
            }
        }, this);
        ...
        $("#redactor_modal_inner").html(this.opts["modal_" + c]);
        $("#redactor_modal_header").html(h);
        if ($("#redactor_tabs").size() !== 0) {
            var f = this;
            $("#redactor_tabs a").each(function(j, k) {
                j++;
                $(k).click(function() {
                    ...
                    $(".redactor_tab").hide();
                    $("#redactor_tab" + j).show();
                    $("#redactor_tab_selected").val(j);
                    if (f.isMobile() === false) {
                        var i = $("#redactor_modal").outerHeight();
                        $("#redactor_modal").css("margin-top", "-" + (i + 10) / 2 + "px")
                    }
                })
            })
        }
        ...
        var a = $("#redactor_modal").outerHeight();
    ...
    setModalTab: function(a) {
        $(".redactor_tab").hide();
        var b = $("#redactor_tabs a");
        b.removeClass("redactor_tabs_act");
        b.eq(a - 1).addClass("redactor_tabs_act");
        $("#redactor_tab" + a).show()
    },
    uploadInit: function(b, a) {
        this.uploadOptions = {
            url: false,
            success: false,
            start: false,
        ...
        $.extend(this.uploadOptions, a);
        if ($("#" + b).size() !== 0 && $("#" + b).get(0).tagName === "INPUT") {
            this.uploadOptions.input = $("#" + b);
            this.element = $($("#" + b).get(0).form)
        } else {
            this.element = $("#" + b)
        }
    ...
    uploadFrame: function() {
        ...
        var b = document.createElement("div");
        var a = '<iframe style="display:none" id="' + this.id + '" name="' + this.id + '"></iframe>';
        b.innerHTML = a;
    ...
    uploadForm: function(d, c) {
        if (this.uploadOptions.input) {
            var e = "redactorUploadForm" + this.id;
            var a = "redactorUploadFile" + this.id;
            this.form = $('<form  action="' + this.uploadOptions.url + '" method="POST" target="' + c + '" name="' + e + '" id="' + e + '" enctype="multipart/form-data"></form>');
            if (this.opts.uploadFields !== false && typeof this.opts.uploadFields === "object") {
                $.each(this.opts.uploadFields, $.proxy(function(h, f) {
                    if (f.indexOf("#") === 0) {
                        f = $(f).val()
                    }
                    var i = $('<input type="hidden" name="' + h + '" value="' + f + '">');
                    $(this.form).append(i)
                }, this))
            }
            var b = this.uploadOptions.input;
            var g = $(b).clone();
            $(b).attr("id", a);
            $(b).before(g);
            $(b).appendTo(this.form);
            ...
            d.attr("action", this.uploadOptions.url);
            this.element.submit()
        }
    },
    uploadLoaded: function() {
        ...
        if (c.contentDocument) {
            ...
            if (c.contentWindow) {
                ...
                e = window.frames[this.id].document
            }
        }
        if (this.uploadOptions.success) {
            if (typeof e !== "undefined") {
                var b = e.body.innerHTML;
                var a = b.match(/\{.*\}/)[0];
                this.uploadOptions.success(a)
            } else {
                alert("Upload failed!");
    ...
    outerHTML: function(a) {
        return $("<p>").append($(a).eq(0).clone()).html()
    },
    normalize: function(a) {
        return parseInt(a.replace("px", ""), 10)
    },
    isMobile: function(a) {
        if (a === true && /(iPhone|iPod|iPad|BlackBerry|Android)/.test(navigator.userAgent)) {
            return true
        } else {
...
$.fn.setCode = function(a) {
    this.data("redactor").setCode(a)
};
$.fn.insertHtml = function(a) {
